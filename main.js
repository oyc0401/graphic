    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (dist < EFFECT_RADIUS) {
                ...
                if (unitX > 0) {
                    // -> 방향으로 밀기, 그러면 왼쪽 픽셀과 현재픽셀 확인
                    if (x == 0) {
                        diffX = displaceX[index]; // or 레프트가 1
                    } else {
                        let leftIdx = y * width + (x - 1);
                        diffX = displaceX[index] + 1 - displaceX[leftIdx];
                    }
                } else {
                    // <-으로 밀기
                    if (x == width - 1) {
                        diffX = -displaceX[index]; // or 라이트가 -1
                    } else {
                        let rightIdx = y * width + (x + 1);
                        diffX = displaceX[rightIdx] + 1 - displaceX[index];
                    }
                }
                ...
                if (unitY > 0) {
                    //아래로 밀기, 위랑 비교
                    if (y == 0) {
                        diffY = displaceY[index]; // or 탑이 1
                    } else {
                        let topIdx = (y - 1) * width + x;
                        diffY = displaceY[index] + 1 - displaceY[topIdx];
                    }
                } else {
                    if (y == height - 1) {
                        diffY = -displaceY[index]; // or 바텀이 -1
                    } else {
                        let bottomIdx = (y + 1) * width + x;
                        diffY = displaceY[bottomIdx] + 1 - displaceY[index];
                    }
                }
