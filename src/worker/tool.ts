...
export function makePaintManager(canvas, gl) {
  let width = paintOptions.width;
  let height = paintOptions.height;

  gl.viewport(0, 0, width, height);
  gl.clearColor(0.9, 0.98, 0.9, 1);
  gl.clear(gl.COLOR_BUFFER_BIT); // 실제로 화면을 그 색상으로 초기화
  ...
  function brush() {
    ...
    // blit 수행
    gl.blitFramebuffer(
      0, 0, paintOptions.width, paintOptions.height,  // src 영역
      0, 0, paintOptions.screenWidth,  paintOptions.screenHeight, // dst 영역
      gl.COLOR_BUFFER_BIT,
