...
export function CapsuleHardModule(
...
class CapsuleHard {
  ...
  move(point: CapsuleHardPoint): CapsuleHardRect | null {
    ...
    const rect = this.drawLine(this.lastPoint, end);
    this.lastPoint = end;
    return rect;
  }
  ...
  private createFullQuadVAO() {
    ...
    if (!buffer || !vao) {
      throw new Error("Failed to create distPixel full quad.");
    }
  ...
  private createAlphaMapTexture() {
    ...
    if (!texture) {
      throw new Error("Failed to create distPixel alpha map texture.");
    }
  ...
  private createFramebuffer(texture: WebGLTexture) {
    ...
    if (!framebuffer) {
      throw new Error("Failed to create distPixel framebuffer.");
    }
    ...
    if (
      ...
      throw new Error("DistPixel framebuffer is incomplete.");
    }
    return framebuffer;
  }
}
...
function createShader(
  ...
  if (!shader) {
    throw new Error("Failed to create distPixel shader.");
  }

  gl.shaderSource(shader, source);
  gl.compileShader(shader);
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    throw new Error(gl.getShaderInfoLog(shader) ?? "DistPixel shader compile failed.");
  }
  return shader;
}
...
function createProgram(
  ...
  if (!program) {
    throw new Error("Failed to create distPixel program.");
  }
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    throw new Error(gl.getProgramInfoLog(program) ?? "DistPixel program link failed.");
  }
  return program;
}
