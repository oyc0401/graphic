export function decodePremultAndFlip(flippedPixel: Uint8Array, width, height): Uint8ClampedArray {
...
export function decodePremultAndFlipWithFlags(
  ...
  for (let row = 0; row < height; row++) {
    for (let col = 0; col < width; col++) {
      ...
      // Base flip: WebGL texture coordinate (bottom-left origin) to standard image (top-left origin)
      let dstRow = height - row - 1;
      let dstCol = col;

      // Apply additional flips if requested
      if (flipV) {
        dstRow = height - dstRow - 1;
      }
      if (flipH) {
        dstCol = width - dstCol - 1;
      }
...
const decodePremultAndFlip2 = (() => {
  return function (flippedPixel: Uint8Array, width: number, height: number): Uint8ClampedArray {
    const pixels = new Uint8ClampedArray(width * height * 4);
